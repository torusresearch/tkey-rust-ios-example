...
//        try! KeychainInterface.syncShare(threshold_key: threshold_key, share_index: nil, curve_n: curve_n)
    ...
    func testSecItemAdd() {
        // Set the password you want to save to the keychain
        let password = "mySuperSecretPassword"

        // Set the keychain attributes
        ...
        let status = SecItemAdd(keychainAttributes as CFDictionary, nil)
        XCTAssertEqual(status, errSecSuccess, "Error saving password to keychain")
    }
}
